...
class ScheduleDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('InnoScheduleID', '{{ $id }}')
            ->rawColumns(['InnoScheduleID', 'action', 'name_data', 'status']);
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('date')->title(trans("Tadbir kuni"))->searchable(false),
            Column::make('started_at')->title(trans("Boshlanish vaqti"))->searchable(false),
            Column::make('stopped_at')->title(trans("Tugash vaqti"))->searchable(false),
            Column::computed('status')->title(trans("Holati"))->searchable(false),
            Column::computed('action')
                ->addClass('action-table-data')
                ->width('1')->title(trans('')),
            Column::computed('InnoScheduleID')
                ->exportable(false)
                ->printable(false)
                ->searchable(false)
                ->visible(false),

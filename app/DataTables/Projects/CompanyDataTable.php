...
class CompanyDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('CompanyID', '{{ $id }}')
            ->rawColumns(['CompanyID', 'action', 'name_data', 'full_name', 'status']);
    }
    ...
    {
        return $model->newQuery()->where([['user_type', 4], ['status', 'active']]);
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('company_name')->exportable(false)->printable(false)->searchable(true)->visible(false),
            Column::make('DT_RowIndex')->title('#')->style('white-space: nowrap;')->addClass('text-center')->searchable(false)->sortable(false),
            Column::computed('name_data')->style('white-space: nowrap; text-align: left;')->title(trans('Tashkilot nomi'))->searchable(false),
            Column::computed('full_name')->style('white-space: nowrap; text-align: left;')->title(trans('Tashkilot rahbari'))->searchable(false),

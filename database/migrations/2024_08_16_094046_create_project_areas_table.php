...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('project_areas', function (Blueprint $table) {
            ...
            $table->enum('status', ['inactive', 'active', 'deleted'])->nullable()->default('activated');
            $table->timestamps();
        });

        Schema::table('project_areas', function (Blueprint $table) {
            $table->foreignId('parent_id')->nullable()->constrained('project_areas')->onUpdate('cascade')->onDelete('cascade');
        });
    }
